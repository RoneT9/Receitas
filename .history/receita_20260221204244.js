...
async function seachRecipes(ingredient) {
  recipelist.innerHTML = `<h4 style="color:green;">carregando receita</h4>`;
  try {
    const response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${ingredient}`);
    const data = await response.json();
    showRecipe(data.meals);
  } catch (error) {
    recipelist.innerHTML = `<h3 style="color:red;">nenhuma receita encontrada 🚫</h3>`;
  }
}
...
async function getRecipeDatails(id) {
  ...
  <p>video: <a href="${recipe.strYoutube}"target="_blank">Assistar no Yobube<p/>`;
}
